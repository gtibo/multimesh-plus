...
func _add_point(point: Dictionary) -> bool:
	var cell_sample_width: int = ceil((point.radius + max_radius) / cell_size)
	
	var cell_sample_count: int = pow(cell_sample_width * 2, 2)
	# Which cell this point is on
	var point_cell: Vector2i = Vector2i(
		floor(point.position.x / cell_size),
		floor(point.position.y / cell_size),
	)

	var current_cell_idx: int = point_cell.x + (cells_row_count * point_cell.y)
